  ...
  <div class="weapons">
    <div class="middle">
      <div class="left" style="display: flex">
        <el-tree class="dialog-tree bar" :data="treeNode" :props="defaultProps" ref="tree" node-key="id" :default-checked-keys="[1]" :current-node-key="1" :default-expand-all="true" @node-click="handleNodeClick">
        </el-tree>
        <el-table ref="singleTable" :data="tableData" border class="allTableStyle">
          <el-table-column property="xh" type="index" label="序号" width="51">
          </el-table-column>
          ...
          <el-table-column property="info" label="操作" width="70">
            <template slot-scope="scope">
              <el-button @click="rowDetail(scope.row)" type="warning" size="mini">详情</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import eMapBar from '@/utils/echarts/aMapBar'
  ...
  methods: {
    initData() {
      Ajax.getAllJson(null).then((res) => {
        ...
        console.log('this.searchData', res.data, this.searchData.length)
        let arr = []
        let xAxisData = []
        let valueData = []
    ...
    handleNodeClick(data) {
      ...
      for (let i = 0; i < this.searchData.length; i++) {
        console.log('data.id', data.id, this.searchData[i].id)
        if (this.searchData[i].id == data.id) {
          console.log("this.tableData", this.tableData, this.searchData[i].data)
